...
parser.add_argument('-c',
                    '--config',
                    default='configs/train_MCGenerator_l1_vgg_gan.json',
                    # default='configs/train_MCGenerator.json',
                    type=str)
...
def main_worker(rank, config):
    ...
    config['save_dir'] = os.path.join(
        config['save_dir'],
        '{}'.format(os.path.basename(args.config).split('.')[0]))
...
if __name__ == "__main__":
    ...
    # mp.spawn(main_worker, nprocs=1, args=(config, ))