...
from .layers.base import get_activation, BaseDiscriminator
from .layers.spatial_transform import LearnableSpatialTransformWrapper
from .layers.squeeze_excitation import SELayer
import os
...
# =================================FreeColorGenerator=================================
class FreeColorGenerator(nn.Module):
    ...
    def forward(self, input):
        # return {'pred':F.interpolate(self.model(input), size=input.shape[-2:], mode='bilinear', align_corners=False)}
        return {'pred':self.model(input)}


# =================================FFCAuxResGenerator=================================
class FreeColorAuxGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,
                 padding_type='reflect', activation_layer=nn.ReLU,
                 up_norm_layer=nn.BatchNorm2d, up_activation=nn.ReLU(True),
                 init_conv_kwargs={}, downsample_conv_kwargs={}, resnet_conv_kwargs={},
                 spatial_transform_layers=None, spatial_transform_kwargs={},
                 add_out_act=True, max_features=1024, out_ffc=False, out_ffc_kwargs={}, adjust_layer=False):
        assert (n_blocks >= 0)
        super().__init__()
        ...
        self.adjust_layer = adjust_layer
    ...
    def get_feature_maps(self, input_data, output_folder='results/feat_map'):
        activations = {}
        
        def make_hook_fn(name):
            def hook_fn(module, input, output):
                # Add output to the dictionary with the module name as the key
                activations[name] = output
            return hook_fn

        # Registering hooks on each layer
        for name, layer in self.named_modules():
            hook_fn = make_hook_fn(name)
            layer.register_forward_hook(hook_fn)

        # Forward pass
        self.forward(input_data)

        # Creating folder for feature maps
        ...
        for layer_name, feature_map in activations.items():
            if isinstance(feature_map, tuple):
                for tuple_idx, tuple_map in enumerate(feature_map):
                    if isinstance(tuple_map, torch.Tensor):
                        feat_map = tuple_map[0].unsqueeze(1)  # Assuming you want to save the first image in batch
                        save_image(feat_map, os.path.join(output_folder, f'{layer_name}_{tuple_idx}.png'), normalize=False)
            else:
                if isinstance(feature_map, torch.Tensor):
                    feat_map = feature_map[0].unsqueeze(1)  # Assuming you want to save the first image in batch
                    save_image(feat_map, os.path.join(output_folder, f'{layer_name}.png'), normalize=False)
...
# =================================FFCAuxEncoder=================================
class FFCAuxEncoder(nn.Module):
    def __init__(self, input_nc=7, output_nc=512, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,
                 padding_type='reflect', activation_layer=nn.ReLU,
                 up_norm_layer=nn.BatchNorm2d, up_activation=nn.ReLU(True),
        ...
        self.concat_tuple = ConcatTupleLayer()
        self.out_fc = nn.Sequential(nn.ReflectionPad2d(3),
                  nn.Conv2d(ngf*8, output_nc, kernel_size=7, padding=0))
    ...
    def forward(self, x):
        ...
        x = self.concat_tuple(x)
        x = self.out_fc(x)
        return {'encode_feat':x, 'aux_downsample_list':aux_downsample_list, 'sizes_after_downsampling':sizes_after_downsampling}


# =================================FFCAuxDecoder=================================
class FFCAuxDecoder(nn.Module):
    def __init__(self, input_nc=512, output_nc=3, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,
                 padding_type='reflect', activation_layer=nn.ReLU,
                 up_norm_layer=nn.BatchNorm2d, up_activation=nn.ReLU(True),
                 init_conv_kwargs={'ratio_gin': 0, 'ratio_gout': 0, 'enable_lfu': False},
                 downsample_conv_kwargs={'ratio_gin': 0, 'ratio_gout': 0, 'enable_lfu': False},
                 resnet_conv_kwargs={'ratio_gin': 0.75, 'ratio_gout': 0.75, 'enable_lfu': False},
                 spatial_transform_layers=None, spatial_transform_kwargs={},
                 add_out_act='sigmoid', max_features=1024, out_ffc=False, out_ffc_kwargs={}, adjust_layer=False):
        assert (n_blocks >= 0)
        super().__init__()
    ...
    def forward(self, x, sizes_after_downsampling, aux_downsample_list):
        # unsampling
        for i, layer in enumerate(self.upsample_modules):
            x = layer(x)
